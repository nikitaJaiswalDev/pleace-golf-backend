...
import { ChampionSchema } from "../models/champions.model";
...
export class CommonService {
        ...
        try {
            ...
            if (createInviteForm.isVerified) {
                if(createInviteForm.isInvitedByDirector) {
                    await this.jobScheduler.now(Job.InvitePlayerJob,{user:createInviteForm});
                }
    ...
    public async getCelebIndexRanking(req: Request, res: Response) {
        const celebrities = await UserSchema.find(
            ...
            { firstName: 1, lastName: 1, nationality: 1, handicapIndex: 1,profession: 1, imageData : 1,gender:1 }
    ...
    public async getClubChamps() {
        try {
            const clubChamps = await GolfClubSchema.find(
                {
                    $or : [
                ...
                { name: 1,countryCode:1,menChamp:1,ladiesChamp:1 }
            ).exec();
    ...
    public async addCountryChampion(req: Request) {
        try {
            const champion: any = req.body;
            if(!champion._id) {
                delete champion._id;
            }
            //await ChampionSchema.deleteOne({division:champion.division,country:champion.country});
            await ChampionSchema.create(champion);
            let champions = await ChampionSchema.find({country : champion.country});
    ...
    public async getCountryChampions(countryCode) {
        try {
            let champions;
            if(countryCode != -1) {
                champions = await ChampionSchema.find({
                    country : countryCode
                });
            } else {
                champions = await ChampionSchema.find({});
    ...
    public async helpSupport(req: Request) {
        try {
            let helpForm: any = req.body;
            this.jobScheduler.now(Job.helpSupportJob,helpForm);
            return Promise.resolve({ successMsg: 'Email is sent to admin' });
        } catch (error) {
            Logger.error(error);
            return Promise.reject(ErrorBuilder.generate(ErrorType.Generic, error));
    ...
    public async getUserTournamentPoints(tournamentId,userId) {
        try {
            let tournamentResult:TournamentResult = await TournamentResultSchema.findOne({
                tournament: ObjectId(tournamentId),
    ...
    public async saveCourseRatings(requestBody,courseId) {
        try {
            //const courseId = '34412fcfef5b4a4e965fcb6f';
            let courseDetails: any = await GolfCourseSchema.findById(courseId).exec();
            ...
            if (courseDetails.courseRatings) {
                updatedCourse = GolfCourseSchema.updateOne({ _id: courseId },
                    {
                ...
                updatedCourse = await GolfCourseSchema.updateOne({ _id: courseId },
                    {
