...
export class TournamentManagementService {
     ...
     */
     public async playNextRound(userID: string, tournamentId: string,): Promise<TournamentScorecard> {
        try {
            ...
            const tournamentScorecards = await this.getAllRoundScorecard(userID,null,tournamentId); //await this.tournamentScorecardDAO.getMultipleByFilters(filters);
            ...
            const storedScorecard = await this.tournamentScorecardDAO.create(nextRoundScorecard);
     ...
     * @returns {Promise<TournamentScorecard[]>} Tournament scorecard.
     */
     public async getAllRoundScorecard(userID: string, scorecardID: string,tournamentID: string): Promise<TournamentScorecard[]> {
        try {
            const tournamentScorecards = await this.tournamentScorecardDAO.getAllRoundScorecard(userID, scorecardID,tournamentID);
     ...
     */
    public async updateScorecard(userID: string, scorecardID: string, scores: Score[],leaderboardId:string): Promise<TournamentScorecard> {
        try {
            ...
            const allRoundScorecards = await this.getAllRoundScorecard(userID,scorecardID,tournamentScorecard.tournamentId);
           ...
           // Logger.info(net);
            ...
            await this.tournamentLeaderboardDAO.updateLeaderboard(userID, tournamentScorecard.tournamentId, tournamentScorecard.course._id,
                 totalPoints, numberOfPlayedHoles,round,currentRound,currentRoundTotal,currentRoundHoles);
     ...
     * @returns {Promise<TournamentLeaderboard>} Tournament leaderboard list.
     */
    public async getLeaderboardById(leaderboardId: string): Promise<TournamentLeaderboard> {
        try {
            const filters = new FilterBuilder()
                .addFilter("_id", leaderboardId)
                .buildAll();
            const leaderboard = await this.tournamentLeaderboardDAO.getByFilters(filters);
            if (leaderboard) {
     ...
     */
    public async createTournamentResult(userID: string, tournamentID: string, courseID: string, division: GolfDivision, total: number, holes: number, round: number): Promise<boolean> {
        try {
    ...
    public async createTournamentResults(tournamentID: string): Promise<boolean> {
        try {
            ...
            const divisionTournamentResults = _.map(tournamentLeaderboards, (tournamentLeaderboard) => {
                let points = playersPoints.find(o => o.userId == tournamentLeaderboard.user)?.points;
                if(tournament.hostingClub && String(tournament.hostingClub._id) == String(tournamentLeaderboard.courseId.clubId._id)){
                    Logger.info('doubling the points of hosting club players');
                    points = 2*points;
                }
    ...
    public async calculatePlayersPoints(tournamentId) {
        ...
        const tournamentLeaderboards = await this.tournamentLeaderboardDAO.getLeaderboardForResults(tournamentId);
