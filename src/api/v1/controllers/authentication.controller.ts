...
export class AuthenticationController {
    ...
    public async login(req: Request, res: Response, next: NextFunction) {
        Logger.info("logging via : "+req.hostname);
        passport.authenticate("local", { session: false }, (err: Error, user: IUser, info: IVerifyOptions) => {
            ...
            } else if(req.hostname === 'pleace-awaken.me' && user.division !== GolfDivision.Celebrity && user.division !== GolfDivision.TourPlayer) {
                ErrorWriter.writeErrorTypeResponse(ErrorType.Authentication, ErrorMessage.CannotLoginInApp, res);
            }
