...
export class TournamentLeaderboardMongoDAO extends MongoDAO<TournamentLeaderboard> implements TournamentLeaderboardDAO {
    ...
    public async updateLeaderboard(userID: string, tournamentID: string, courseID: string, total: number, holes: number, round: number,
                    currentRound:number,currentRoundTotal:number,currentRoundHoles:number): Promise<TournamentLeaderboard> {
        try {
    ...
    public async getLeaderboardForResults(tournamentID: string): Promise<TournamentLeaderboard[]> {
        try {
            return TournamentLeaderboardSchema.find(
                {
                    tournamentId: tournamentID,
            ...
            Logger.error(`Could not get tournament leaderboard ${tournamentID} Error: ${error}`);
            throw error;
        }
    }
